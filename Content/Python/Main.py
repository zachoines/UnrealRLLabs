...
import torch
from Agents import REINFORCELearner, Config
...
class SharedMemoryInterface:
    ...
    def _fetch_config_from_shared_memory(self):
        ...
        return Config(num_environments=num_environments, 
    ...
    def get_experiences(self):
        ...
        transition_length = (2 * self.config.state_size) + self.config.num_actions + 2  # +2 for reward and done
        
        # Reshape the array to shape (num_envs, num_steps, transition_length)
        update_array = update_array.reshape(self.config.num_environments, self.config.training_batch_size, transition_length)
        
        # Split the array into states, next_states, actions, rewards, and dones
        states = update_array[:, :, :self.config.state_size]
        next_states = update_array[:, :, self.config.state_size:2 * self.config.state_size]
        actions = update_array[:, :, 2 * self.config.state_size:2 * self.config.state_size + self.config.num_actions]
...
if __name__ == "__main__":
    ...
    agent = REINFORCELearner(memory_interface.config)
