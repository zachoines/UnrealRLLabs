...
class RLRunner:
    ...
    def __init__(self, agent: Agent, agentComm: EnvCommunicationInterface, config: Dict) -> None:
        ...
        self.device = self.agentComm.device # Correctly define self.device
        ...
        if self.enable_memory:
            self.current_memory_hidden_states = torch.zeros(
                ...
                device=self.device # Ensure tensors are on the correct device
            )
        else:
        ...
        self.current_segments_per_env: List[TrajectorySegment] = [
            TrajectorySegment(self.num_agents_max_config, self.device, self.pad_trajectories, self.sequence_length)
                ...
                if self.enable_memory:
                    for env_idx in range(self.num_environments):
                        ...
                        if dones_ue[env_idx].item() > 0.5 or truncs_ue[env_idx].item() > 0.5:
                            ...
                            self.current_segments_per_env[env_idx] = TrajectorySegment(self.num_agents_max_config, self.device, self.pad_trajectories, self.sequence_length)
    ...
    def _collate_and_pad_sequences(self, trajectory_segments: List[TrajectorySegment]) \
        ...
        if not trajectory_segments:
            ...
            empty_state_dict = {}
            return empty_state_dict, torch.empty(0, device=self.device), torch.empty(0, device=self.device), \
                   empty_state_dict, torch.empty(0, device=self.device), torch.empty(0, device=self.device), \
                   None, torch.empty(0, device=self.device)
        ...
        batch_dim_size = len(collated_true_lengths) # This is the number of segments in the batch

        final_padded_act = torch.stack(collated_act_seqs, dim=0) if collated_act_seqs else torch.empty(batch_dim_size, target_len_for_batch, self.num_agents_max_config, 0, device=self.device)
        final_padded_rew = torch.stack(collated_rew_seqs, dim=0) if collated_rew_seqs else torch.empty(batch_dim_size, target_len_for_batch, 1, device=self.device)
        final_padded_done = torch.stack(collated_done_seqs, dim=0) if collated_done_seqs else torch.empty(batch_dim_size, target_len_for_batch, 1, device=self.device)
        final_padded_trunc = torch.stack(collated_trunc_seqs, dim=0) if collated_trunc_seqs else torch.empty(batch_dim_size, target_len_for_batch, 1, device=self.device)
        ...
        # Use batch_dim_size (number of segments) for the first dimension of attention_mask
        attention_mask = torch.zeros(batch_dim_size, target_len_for_batch, device=self.device, dtype=torch.float32)
