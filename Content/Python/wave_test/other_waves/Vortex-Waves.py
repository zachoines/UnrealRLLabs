...
# Coherent Vortex Wavelet Parameters
...
# Initialize radial wavenumbers
agent_radial_wavenumbers = np.random.uniform(0.2, 0.5, N_AGENTS)

# Initialize angular frequencies
agent_frequencies = np.random.uniform(0.05, 0.2, N_AGENTS)

# Initialize angular factors (theta components)
agent_angular_factors = np.random.uniform(0.1, 0.5, N_AGENTS)
...
def update_plot(frame_number):
    global agent_positions, agent_directions, agent_speeds, agent_amplitudes, agent_radial_wavenumbers, agent_frequencies, agent_phases, agent_angular_factors, heights
    ...
    for a in range(N_AGENTS):
        ...
        k_r = agent_radial_wavenumbers[a]
        omega_a = agent_frequencies[a]
        phi_a = agent_phases[a]
        theta_a = agent_angular_factors[a]
        ...
        # Compute the Coherent Vortex Wavelet
        wave = A_a * envelope * np.cos(k_r * r + omega_a * time + phi_a + theta_a * r)
