...
UDiscreteFourier2D::UDiscreteFourier2D()
{
    ...
    GridSizeY = 0;
    K = 0;
    MatrixDeltaRange = FVector2D(-0.05f, 0.05f); // default
}
...
const FMatrix2D& UDiscreteFourier2D::Update(const TArray<FAgentFourierDelta>& Deltas)
{
    ...
    for (int32 i = 0; i < Deltas.Num(); ++i)
    {
        ...
        newVal = FMath::Clamp(newVal, MatrixDeltaRange.X, MatrixDeltaRange.Y);
...
const FMatrix2D& UDiscreteFourier2D::GetDeltaHeights() const
{
    return DeltaHeights;
}
...
TArray<float> UDiscreteFourier2D::GetAgentFourierState(int32 AgentIndex) const
{
    ...
    // 1) flatten AgentA
    int32 R = A.AgentA.GetNumRows();
    int32 C = A.AgentA.GetNumColumns();
    for (int32 rr = 0; rr < R; ++rr)
    {
        for (int32 cc = 0; cc < C; ++cc)
        {
            Out.Add(A.AgentA[rr][cc]);
        }
    }
